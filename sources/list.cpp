...
namespace hc2img {
    ...
    void draw_clef(cimg_library::CImg<unsigned char> &image, harmony_core::Clef clef, uint16_t mutch) {
    }

    void draw_staff(cimg_library::CImg<unsigned char> &image, uint16_t mutch) {
        for (int x = 0; x < mutch; ++x) {
            for (int y = 0; y < 5; ++y) {
                image.draw_line(staff_edge_gap, staff_edge_gap + y * staff_line_gap + x * staff_gap,
                                list_size_x - staff_edge_gap, staff_edge_gap + y * staff_line_gap + x * staff_gap,
                                black, 1.0f);
            }
        }
    }
    ...
    void List::draw() noexcept {
        ...
        cimg_library::CImg<unsigned char> overlay_image("img/treble_clef.png");

        image = image + overlay_image;
