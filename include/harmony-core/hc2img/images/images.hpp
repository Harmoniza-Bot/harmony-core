 ...
 * Каждая структура имеет методы x(int), y(int) и size(), возвращающие точки для отрисовки и количество точек
 * соответственно. Также у нот есть отдельная функция, возвращающая координату для крепления штиля.
 */
namespace images {}
